...
//        break;
  ...
  <script type="text/javascript">
    function HP_FS(holderName,currentName,key,xmlPath) {
      this.CallBack = function() {
        if (rmc.Xml[key] != null && rmc.Xml[key].readyState == 4) {
          var xml = rmc.Xml[key].responseXML;
          if (xml == undefined) { xml = rmc.Xml[key]; }
          if (typeof (xml) == "object") {
            xml.documentElement.setAttribute("prefix", prefix);
            var html = rmc.Transform(xml, rmc.Xsl[key]);
      ...
      this.Show = function(delta) {
        var t = ((t = currentItem + delta) > max) ? 1 : ((t < 1) ? max : t);
        if (currentItem != 0) {
          var n = rmc.GE(prefix + currentItem);
          if (n != null) { n.style.display = "none"; }
        }
        var n = rmc.GE(prefix + t);
        if (n != null) { n.style.display = "block"; }
        currentItem = t;
        currentHolder.innerHTML = "(" + t + "/" + max + ")";
      };

      var currentItem = 0;
      var prefix = key+"i";
      var _this = this;
      var holder = rmc.GE(holderName);
      var currentHolder = rmc.GE(currentName);
      var max = 3;
      rmc.AddXsl(key, "/xsl/hp_fs.xsl");
      rmc.AddXml(key, xmlPath, _this.CallBack, true);
    }

    function HP_TK(n,holderName,itemName,itemHolder,key,xmlPath) {
      ...
      this.CallBack = function() {
        if (rmc.Xml[key] != null && rmc.Xml[key].readyState == 4) {
          ...
          if (typeof (xml) == 'object') {
            xml.documentElement.setAttribute("prefix", prefix);
            xml.documentElement.setAttribute("maxItems", maxItems);
            var html = rmc.Transform(xml, rmc.Xsl[key]);
            holder.innerHTML = html;
            _this.Clear();
          }
      ...
      this.Next = function() {
        current = (++current > maxItems) ? 1 : current;
        var data = rmc.GE(prefix + current);
        if (data == null) { return; }
        tickerItem.style.left = startPos;
        tickerItem.innerHTML = data.innerHTML;
        tickerItem.style.display = "block";
        if (tickerItem.clientWidth > maxWidth) {
          sliding = true;
          amountToMove = tickerItem.clientWidth - maxWidth;
          tMove = setTimeout(n + ".Slide();", 2000);
        }
        else {
          tMove = setTimeout(n + ".Clear();", 5000);
        }
      };

      this.Clear = function() {
        tickerItem.style.display = "none";
        setTimeout(n + ".Next();", 500);
      };
      ...
      this.Slide = function() {
        if (Math.abs(tickerItem.offsetLeft) >= amountToMove) {
          sliding = false;
          tMove = setTimeout(n + ".Clear();", 2000);
        }
        else {
          sliding = true;
          tickerItem.style.left = (parseInt(tickerItem.style.left) + perSlice) + "px";
          tMove = setTimeout(n + ".Slide();", 10);
        }
      };
      ...
      this.Out = function() {
        if (sliding) {
          tMove = setTimeout(n + ".Slide();", 1000);
        }
        else {
          tMove = setTimeout(n + ".Clear();", 3000);
        }
      }

      var holder = rmc.GE(holderName);
      var tickerItem = rmc.GE(itemName);
      var tickerDiv = rmc.GE(itemHolder);
      rmc.AE(tickerDiv, "mouseover", this.Over);
      rmc.AE(tickerDiv, "mouseout", this.Out);

      var tMove = null;
      var perSlice = -1;
      var startPos = tickerItem.offsetLeft + "px";
      var maxWidth = tickerDiv.clientWidth - tickerItem.offsetLeft;
      var amountToMove;
      var sliding = false;
  ...
  <style type="text/css">
    ...
    #holder {position:relative;top:0px;left:0px;width:978px;height:695px;margin-top:4px;cursor:default;}  
    
    #mainGraphic {position:absolute;top:6px;left:6px;width:642px;height:397px;overflow:hidden;}
    #mainGraphic img {width:642px;height:397px;}    
    
    #featuredStories {position:absolute;top:6px;left:654px;width:318px;height:430px;background:url(/a/i/hp/fs_bkg.gif) repeat-x;overflow:hidden;}
    #featuredStories #fsTop h1 {position:absolute;top:7px;left:12px;font:16px verdana;color:#018;margin:0px;}
    #featuredStories #fsTop h1 span {font:11px verdana;color:#323223;}
    #featuredStories #fsTop h2 {position:absolute;top:9px;left:278px;margin:0px;}
    #featuredStories #fsTop h2 img {margin-right:2px;cursor:pointer;vertical-align:top;}
    #featuredStories #fsDiv {position:absolute;top:30px;left:12px;width:297px;height:374px;overflow:hidden;}
    #featuredStories #fsDiv img {width:294px;margin-bottom:6px;border:none;}
    #featuredStories #fsDiv h1 {margin:0px 0px 3px 0px;font:16px georgia;color:#16f;}
	#featuredStories #fsDiv h1 a:visited {color:#16f;}
    #featuredStories #fsDiv h2 {margin:0px;font:13px verdana;color:#777; line-height:1.5;}
    #featuredStories #fsBot {position:absolute;top:403px;text-align:right;width:309px;height:27px;font:13px verdana; padding-top:5px;}
	#featuredStories #fsBot a:hover{text-decoration:underline;}
	#featuredStories #fsBot a:visited {color:#16f;}
    #featuredStories #fsBot img {width:13px;height:13px;vertical-align:bottom;}
    
    #ticker {position:absolute;top:409px;left:6px;width:642px;height:27px;background:url(/apps/dp/i/shade_dn.png) repeat-x;overflow:hidden;}
    #ticker img {position:absolute;top:8px;left:617px;width:13px;height:13px;cursor:pointer;}
    #ticker #label {position:absolute;top:5px;left:8px;width:46px;height:18px;border-right:1px solid #e70;font:bold 13px verdana;}
    #ticker #label a {color:#e70; text-decoration:none;}
    #ticker #tickerDiv {position:absolute;top:0px;left:66px;width:545px;height:27px;overflow:hidden;}
    #ticker #tickerDiv #tickerItem {position:absolute;top:5px;white-space:nowrap;font:13px verdana;}
    #ticker #tickerDiv #tickerItem a {font:13px verdana;color:#323223;text-decoration:none;}
    
    #tickerDD {display:none;position:absolute;top:436px;left:61px;width:588px;z-index:100;background: #efeeee url(/a/i/hp/drop.png) repeat-x;cursor:default;}
    #tickerDD a {font:13px verdana;color:#323223;line-height:1.5;text-decoration:none;}
	#tickerDD a:visited {color:#323223;}
    #tickerDD .bot {margin-top:12px;}
    #tickerDD #tickerDDItems div, #tickerDD .bot {padding:5px 10px 8px 10px;}
    #tickerDD #tickerDDItems div:hover, #tickerDD .bot:hover {background:#16f;}
    #tickerDD #tickerDDItems div:hover a, #tickerDD .bot:hover a {color:#fff;text-decoration:none;}
    #tickerDD > div {border-left:1px solid #ddd;border-right:1px solid #ddd;}
    #tickerDD b {display:block;height:1px;}
    #tickerDD b.r1 {display:block;background:#9f9f9f;margin:0 1px;}
    #tickerDD b.r2 {display:block;background:#ddd;margin:0 2px;}
	
	#sectionOne h5 {font-family:verdana; font-size:16px; text-transform:capitalize; letter-spacing:0;}
	#sectionTwo h5 {font-family:verdana; font-size:16px; text-transform:capitalize;letter-spacing:0;}
	#sectionThree h5 {font-family:verdana; font-size:16px; text-transform:capitalize;letter-spacing:0;}
    
    #sectionOne {position:absolute;top:442px;left:6px;width:306px;height:246px;overflow:hidden;padding:6px 0px 0px 12px;}
	#sectionOne li {background-image:url(/a/i/hp/ql.png); background-repeat:no-repeat; background-position:6px 4px;list-style-type:none; padding-left:21px;}
		/*#sectionOne img {position:absolute;top:60px;left:42px;}*/
		#sectionOne ul {padding:9px 0px 9px 0px;}
		#sectionOne ul a:visited{ color:#16f;}
		#sectionOne div div {border-bottom:1px solid #ccc;margin-right:12px;}
		#sectionOne div div:first-child {border-top:1px solid #ccc; margin-top:18px;}

		#sectionTwo {position:absolute;top:442px;left:330px;width:306px;height:246px;overflow:hidden;padding:6px 0px 0px 12px;}
		#sectionTwo p {position:absolute;top:30px;left:12px;}
		#sectionTwo img {position:absolute;top:42px;left:13px;}
    #sectionThree {position:absolute;top:442px;left:654px;width:306px;height:246px;overflow:hidden;padding:6px 0px 0px 12px;}
    #sectionThree img {margin-top:21px;cursor:pointer;}
		#sectionThree #asia{position:absolute;top:85px;left:247px;overflow:hidden;padding:0px;}
		#sectionThree #cambridge{position:absolute;top:70px;left:148px;overflow:hidden;padding:0px;}
		#sectionThree #india{position:absolute;top:112px;left:217px;overflow:hidden;padding:0px;}
		#sectionThree #newengland{position:absolute;top:85px;left:91px;overflow:hidden;padding:0px;}
		#sectionThree #siliconvalley{position:absolute;top:88px;left:46px;overflow:hidden;padding:0px;}
		#sectionThree #redmond{position:absolute;top:79px;left:47px;overflow:hidden;padding:0px;}
	
	
    .home .section {background:url(/a/i/hp/row2_bkg.gif) repeat-x;}
  </style>
</head>
		  ...
		  <a name="rmcTop"></a>
			...
			<div id="shadow" class="center sw">
              ...
              <table cellspacing="0" cellpadding="0" border="0">
                ...
                <tr>
                  <td>&nbsp;</td>
      		            <div id="ctl00_ctl00_Navigation2_holder"><td class="sel">
...
<td>
  ...
  <li class="first"><a href="/c/1001" onClick="stc(this, 13)">About Us</a></li>
  <li class=""><a href="/c/1002" onClick="stc(this, 14)">News</a></li>
  <li class=""><a href="/c/1003" onClick="stc(this, 15)">Media Resources</a></li>
  <li class=""><a href="/c/1004" onClick="stc(this, 16)">Events</a></li>
  <li class="last"><a href="/c/1005" onClick="stc(this, 17)">Community</a></li>
</ul></div>
</div>
      		  <div id="chPM"><div id="ctl00_ctl00_ctl00_holder"><table cellpadding="0" cellspacing="0" border="0" id="pmleft"><tr><td valign="top"><a href="/c/1204" onClick="stc(this, 18)"><img border="0" src="/en-us/images/ads/chrome/featured_videos_chrome_ad_3.png" alt="Featured Videos from Microsoft Research" title="Featured Videos from Microsoft Research"></a></td></tr></table></div>
</div>
    		  </div>
					<div id="crumbBar">
						<div id="tools">
	<ul class="horiz">
	    <li>
	        ...
	        <a href="/c/1057" onClick="stc(this, 19)"><img title="Read the Inside Microsoft Research blog" alt="Read the Inside Microsoft Research blog" src="/a/i/c/blog_16x16.png" border="0" width="16" height="16" /></a>
	    </li>
	    <li>
	    ...
	    <li class="last">
	        <a href="/c/1046" onClick="stc(this, 20)"><img title="RSS feeds" alt="RSS feeds" src="/a/i/c/rss.gif" border="0" width="18" height="18" /></a>
	    </li>
	</ul>
</div>
						<a href="/c/1047" onClick="stc(this, 21)"><img alt="Home" border="0" src="/a/i/c/bc_home.png" /></a>
						<span id="bcHolder">
  ...
  <div id="holder">
    <div id="mainGraphic">
	<a href="/apps/c/1078.aspx?77844" onClick="stc(this, 22)"><img src="/en-us/images/ads/hero/oscars_hero_642x397.png" border="0" alt="Predicting the 2013 Oscar winners" title="Predicting the 2013 Oscar winners" /></a>
    </div>
    <div id="featuredStories">
      <div id="fsTop">
	      <h1>Feature Stories <span id="fsCurrent"></span></h1>
	      <h2><img src="/a/i/hp/l_btn.gif" onclick="fso.Show(-1);"/><img src="/a/i/hp/r_btn.gif" onclick="fso.Show(1);"/></h2>
	    </div>
	    <div id="fsDiv"></div>
	    <div id="fsBot"><a href="/c/1110" onClick="stc(this, 23)">More feature stories</a> <a href="/c/1110" onClick="stc(this, 24)"><img border="0" src="/a/i/hp/more.gif" /></a></div>
	    <script type="text/javascript">
	        var fso = new HP_FS("fsDiv", "fsCurrent", "fs", "/rss/featuredstories.xml?" + Math.floor(Math.random() * 1001));
	    </script>
    </div>
    <div id="ticker">
      <img id="tickerBtn" src="/a/i/hp/dn_btn.gif" />
      <div id="label"><a href="/c/1131" onClick="stc(this, 25)">News</a></div>
      <div id="tickerDiv">
        <div id="tickerItem"></div>
      </div>
    </div>
    <div id="tickerDD">
      <div>
        <div id="tickerDDItems"></div>
        <div class="bot"><a href="/c/1130" onClick="stc(this, 26)">See all news</a></div>
      </div>
      <b class="r1"></b>
      <b class="r2"></b>
      <script type="text/javascript">
        var tko = new HP_TK("tko", "tickerDDItems", "tickerItem", "tickerDiv", "tk", "/rss/newsandhighlights.xml?" + Math.floor(Math.random() * 1001));
        rmc.NewHover("tickerBtn", "tickerDD",0,0,false);
      </script>
    </div>
    <div id="sectionOne" class="section">
      <h5>Highlights</h5>
      <div><div><ul><li><a href="/apps/c/1070.aspx?98313" onClick="stc(this, 27)">Find Scholarly Content with Academic Search</a></li></ul></div><div><ul><li><a href="/apps/c/1071.aspx?82377" onClick="stc(this, 28)">Download the Image Composite Editor</a></li></ul></div><div><ul><li><a href="/apps/c/1072.aspx?47994" onClick="stc(this, 29)">Top Academic and Scientific Downloads</a></li></ul></div><div><ul><li><a href="/apps/c/1073.aspx?22532" onClick="stc(this, 30)">Merge Photos with AutoCollage</a></li></ul></div></div>
    </div>
    <div id="sectionTwo" class="section">
      <h5>Spotlight</h5>
      <a href="/apps/c/1076.aspx?82723" onClick="stc(this, 31)"><img src="/en-us/images/ads/try-f-sharp_spot.jpg" border="0" alt="Try F# simplifies code for complex problems" title="Try F# simplifies code for complex problems" /></a>
    </div>
   <div id="sectionThree" class="section">
      <h5>Worldwide Locations</h5>
<map id="ImgMap1" name="ImgMap1">
<area href="/c/1140" coords="17, 27, 65, 40" shape="rect" / onClick="stc(this, 32)">
<area href="/c/1141" coords="6, 56, 68, 69" shape="rect" / onClick="stc(this, 33)">
<area href="/c/1142" coords="2, 12, 27, 25" shape="rect" / onClick="stc(this, 34)">
<area href="/c/1143" coords="72, 31, 133, 44" shape="rect" / onClick="stc(this, 35)">
<area href="/c/1144" coords="139, 22, 193, 35" shape="rect" / onClick="stc(this, 36)">
<area href="/c/1145" coords="207, 64, 234, 77" shape="rect" / onClick="stc(this, 37)">
<area href="/c/1146" coords="217, 37, 244, 50" shape="rect" / onClick="stc(this, 38)">
<area href="/c/1147" coords="99, 60, 156, 73" shape="rect" / onClick="stc(this, 39)">
<area href="/c/1148"  coords="137, 89, 184, 102" shape="rect" / onClick="stc(this, 40)">
<area href="/c/1149" coords="30, 12, 80, 25" shape="rect" / onClick="stc(this, 41)">
<area href="/c/1150" coords="159, 50, 204, 63" shape="rect" / onClick="stc(this, 42)">
<area href="/c/1190" coords="70, 75, 117, 88" shape="rect" / onClick="stc(this, 43)">
<area href="/c/1191" coords="14, 93, 71, 106" shape="rect" / onClick="stc(this, 44)">
</map>



<img border="0" src="/en-us/images/world_map_small.png" width="302" height="170" usemap="#ImgMap1">
			...
			<div id="ctl00_ctl00_ctl01_holder" class="shellFooter"><div id="footer" class="center sw"><div class="fr"><span>&copy;2013 Microsoft Corporation. All rights reserved.</span><a href="/c/1064" onClick="stc(this, 45)"><img alt="Microsoft" title="Microsoft" src="/a/i/c/logo_ms.png" border="0"></a></div><ul class="horiz"><li class="first"><a href="/c/1060" onClick="stc(this, 46)">Contact</a></li><li class=""><a href="/c/1061" onClick="stc(this, 47)">Terms</a></li><li class=""><a href="/c/1062" onClick="stc(this, 48)">Trademarks</a></li><li class=""><a href="/c/1063" onClick="stc(this, 49)">Privacy and Cookies</a></li><li class=""><a href="/c/1065" onClick="stc(this, 50)">Code of Conduct</a></li><li class="last"><a href="javascript:OpenFeedback();">Feedback</a></li></ul></div></div>
